...
**Base inspeccionada:** [`8a61dad`](https://github.com/dnluc/decilo/tree/8a61dadc6ca244b678d12aef7d7cee8875f10f02), incluyendo el PR #20 de ASR incremental y autodetección de idioma.  
...
| Texto provisional | Retranscripción del segmento abierto; la pasada final lo reemplaza |
| Cortes variables | Pausas acústicas con mínimo y máximo; sin detector semántico textual |
| Traducción progresiva | Opcional, NDJSON de Ollama sobre originales finales |
| Gemini | `generateContent` por solicitud; default `gemini-3.1-flash-lite` |
...
| Calidad de frases | Segmentar por pausa y confirmar con otra pasada de ASR | La pausa no prueba que terminó una idea; el máximo puede cortar una oración |
...
flowchart LR
    ...
    API <-->|"HTTPS: audio o texto"| Gemini["Gemini API: nube"]
```
...
| Gemini | HTTPS externo | STT y/o traducción según configuración |
...
flowchart TB
    ...
    Ingest --> Segment["segmentation.py: límites acústicos"]
...
| [segmentation.py](../src/decilo/segmentation.py) | Política acústica pura sobre PCM; no llama modelos |
...
| [gemini.py](../src/decilo/gemini.py) | Payload, autenticación, timeouts y validación de respuestas de Gemini |
...
La separación permite que muchos espectadores compartan los resultados de una única inferencia. Abrir otra página de audiencia no retranscribe la sesión.
...
flowchart LR
    ...
    WS --> Decode["Validación y segmentación en Python"]
...
El backend acepta entre 1 y 80.000 muestras por paquete, exige offset sin retrocesos/solapamientos y limita el final a una hora de muestras. Un salto de offset representa audio faltante: se cierra el segmento previo, se publica un gap y se reinicia el segmentador en el nuevo offset.

Los tiempos de subtítulos se calculan con `muestras × 1000 / 16000`. Son relativos a la captura, **no al minuto absoluto del video de YouTube**. El navegador y Python no comparten un reloj monotónico.
...
No hay todavía detector de sintaxis, puntuación estable o significado. El contrato admite `boundary_reason=semantic`, pero el segmentador actual no lo produce.

### 7.2 Hipótesis del segmento abierto

Con segmentación por pausas y `DECILO_PARTIALS=1` —default actual— la captura activa `PartialTranscriber`:
...
- El proveedor local recibe `beam_size=1` para parciales y `beam_size=5` para la pasada final.
...
Los parciales son originales; no se traducen especulativamente. La traducción empieza después del original final. En Gemini, esta misma estrategia provoca solicitudes completas repetidas de `generateContent`: no es streaming nativo de audio y `beam_size` no tiene efecto en ese adaptador. Puede aumentar costo y contención. `DECILO_PARTIALS=0` desactiva el experimento.
...
flowchart TD
    ...
    Dispatch -->|gemini| Cloud["Gemini generateContent"]
```
...
`GET /api/v1/providers` publica `default` y `cloud_available`. La segunda propiedad indica presencia de una clave, no valida saldo, cuota ni acceso al modelo. La UI guarda su selección en `localStorage`; sin preferencia guardada empieza en local. Actualmente no aplica automáticamente el campo `default` del backend al selector.

### 8.2 Camino local

**STT:** faster-whisper se carga de forma diferida dentro del proceso Python. Usa CPU, cuantización `int8`, modelo `small` para EN y `medium` para ES. Un lock por tamaño evita construir dos copias del mismo modelo en un arranque concurrente. Ese lock protege la carga, no serializa toda la inferencia.
...
El modelo predeterminado es `gemini-3.1-flash-lite`, reemplazable por `DECILO_GEMINI_MODEL`. Para STT se envía un WAV en base64 mediante `inlineData`; para traducción, texto. La clave viaja en el header `x-goog-api-key`, nunca en la URL ni al frontend.

Cada solicitud usa temperatura 0 y máximo 2.048 tokens de salida. Solo se acepta una respuesta con `finishReason=STOP` y texto no vacío; se excluyen partes marcadas como pensamiento. El adaptador de audio rechaza archivos de más de 2.000.000 bytes.

Hay timeout HTTP de 30 s, sin reintentos ni fallback automático. Cada llamada crea su propio cliente HTTP; a diferencia de Ollama, Gemini no reutiliza un cliente del lifespan. La llamada STT síncrona corre en un thread y la traducción usa cliente async. Es `generateContent`, **no Gemini Live API**.
...
Con `DECILO_TRANSLATION_QUEUE=1`, ASR puede avanzar mientras se traduce un original anterior. Hay hasta dos textos pendientes más una traducción activa. Al llenarse la cola, `await queue.put` espera: la cola no crea solicitudes ilimitadas. Ese freno puede terminar llenando la cola anterior de audio, donde sí hay descarte.
...
Cada captura puede tener recepción, un worker de ASR final, un worker provisional y un consumidor de traducción opcional. El provisional y el final son caminos concurrentes distintos; **no hay una única inferencia global serializada**. Comparten thread pool, CPU, caché de Whisper y los servicios externos.
...
| ASR provisional | Un activo y una instantánea pendiente | Reemplaza la pendiente; no cancela la inferencia activa |
...
| `DECILO_GEMINI_MODEL` | `gemini-3.1-flash-lite` | Modelo para ambas funciones del adaptador Gemini |
| `DECILO_PREWARM` | Deshabilitado si no es `1` | Prepara Whisper EN/ES y Ollama en segundo plano |
| `DECILO_PARTIALS` | `1` | ASR provisional en captura con segmentación por pausas |
...
`disabled` significa «sin precarga», no «modelos comprobados». Una captura local puede cargar el modelo con su primer audio. La precarga es local aun con Gemini seleccionado globalmente. Una falla de precarga no bloquea una captura explícita de nube con idioma fijo; `/health/ready` sigue describiendo la preparación local, no la disponibilidad integral de todos los proveedores.
...
En modo nube, Gemini recibe audio para STT y texto para traducción. Las claves permanecen en backend, `.env` no se versiona y el catálogo solo informa su presencia. La inferencia crea WAV temporales; no hay almacenamiento deliberado de la charla completa, exportación ni política de archivo persistente.
...
| --- | --- | --- |
| Límites por unidades de sentido | `segmentation.py`, hipótesis de `partials.py`, `boundary_reason` | Detector textual, estabilidad y plazo absoluto verificable |
...
| Proveedores y aislamiento | `test_providers.py`, `test_provider_selection.py` |
...
`tests/conftest.py` aísla las pruebas sin modelos del `.env` real y bloquea transportes HTTP de inferencia. La integración utiliza servidor, WebSockets y worklet reales con inferencia simulada. Eso verifica contratos y recorridos, no precisión del ASR ni calidad de traducción.
...
- [Prueba mínima de nube](validation/2026-09-25-cloud-backend/README.md): un fragmento sintético con Gemini **3.8 Flash**, anterior al cambio de modelo. No es un benchmark de 3.1 Flash-Lite.
- [Avance incremental de OpenSpec](../openspec/changes/backend-latencia-incremental/tasks.md): smoke y tests registrados por Claude para el PR #20; evaluación sostenida pendiente.
